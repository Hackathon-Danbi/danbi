...
export function getPracticeStepGuidance(
  ...
  options?: { accountReview?: boolean },
): string | null {
  if (style !== 'guided') return null;
  // 계좌번호 맞춤 복습은 받는 사람 화면을 재사용하지만, 하는 일은 계좌번호 입력이다.
  if (options?.accountReview && screen === 'practiceRecipient') {
    return '안내된 계좌번호를 보고 숫자로 똑같이 입력해주세요.';
  }
